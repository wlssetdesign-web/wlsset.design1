          ...
          {activeProject.title === "Khalil Barber Shop" ? (
            ...
            <div className="pt-32 pb-12">
              {activeProject.title !== "Dry Eyes Problems" && activeProject.title !== "تواصل" && (
                <div className="flex items-center justify-center px-4 py-20 m-0" style={{ backgroundColor: activeProject.title === "Kick Off" ? "#A51E1E" : activeProject.title === "مناحل نابلس" ? "#E8C832" : activeProject.title === "Al Mamlaka TV NFC" ? "#8B1A1A" : activeProject.title === "Meet Me Again" ? "#875c49" : activeProject.title === "Honey Masters.jo" ? "#F0F4E8" : "#1D695A" }}>
                  <div className="max-w-5xl w-full">
                    <p className={`text-xl md:text-2xl font-bold leading-relaxed ${activeProject.title === "مناحل نابلس" ? "text-[#333333]" : activeProject.title === "Honey Masters.jo" ? "text-[#3B2A1A]" : "text-white"}`}>
                      {activeProject.title === "مناحل نابلس" ? "logo features four geometric shapes arranged in a honeycomb-inspired grid, representing the structure of a beehive. The warm golden yellow color directly reflects the natural tone of pure honey, while the modern Arabic typography grounds the design with elegance. A perfect blend of tradition and contemporary style." : activeProject.title === "Al Mamlaka TV NFC" ? <><span className="font-extrabold">Al Mamlaka TV NFC</span>{" \u2014 A bold identity card designed for power and presence, crafted exclusively for the CEO of Al Mamlaka TV. The pixelated red and gray pattern bursts from the corners like a digital signal, reflecting the brand's media-driven DNA. Clean white space at the center keeps the focus sharp on what matters \u2014 the name, the title, the legacy. Embedded with NFC technology, one tap or scan is all it takes for the CEO to make a lasting connection."}</> : activeProject.title === "Meet Me Again" ? "Meet Me Again coffee packaging stickers designed with a warm, earthy identity. Each label blends bilingual typography with origin details, flavor notes, and a personal touch \u2014 crafted to tell the story of every bean before the first sip." : `${activeProject.title} | ${activeProject.description}`}
                    </p>
                  </div>
                </div>
              )}
